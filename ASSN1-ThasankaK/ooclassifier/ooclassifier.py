...
class TrainingInstance(C274):
    ...
    def preprocess_words(self, mode=''):
        self.lowercase()
        self.delSymbols(mode)
        self.delDigits(mode)
        self.delStops(mode)

    def lowercase(self):
        x = ' '.join([i.lower() for i in self.inst["words"]]).split()
        self.inst["words"] = x
        # joins into a string of lowercase, then back to a list

    def delDigits(self, mode):
        temp = []
        if mode != 'keep-digits':
            for i in self.inst["words"]:
                if any(char.isdigit() for char in i):
                    ...
                    temp.append(i)
                    # this means it has no numbers
            self.inst["words"] = temp

    def delSymbols(self, mode):
        if mode != 'keep-symbols':
            x = ' '.join(self.inst["words"])
            for i in ".,?!;:'-_()[]"".../|*@#$%^&":
                x = x.replace(i, '')
            self.inst["words"] = x.split()

    def delStops(self, mode):
        if mode != 'keep-stops':
            temp = []
            for k in self.inst["words"]:
...
class TrainingSet(C274):
    ...
    def return_nfolds(self, num=3):
        tempHash = [self.inObjHash[i::num] for i in range(num)]
        tempList = [self.inObjList[j::num] for j in range(num)]
        x = self.copy()
        objList = [[]]*num
        for i in range(num):
            x = self.copy()
            x.inObjHash = tempHash[i]
            x.inObjList = tempList[i]
            objList[i] = x
        return objList
    ...
    def add_training_set(self, tset):
        temp = tset.copy()
        self.inObjHash.extend(temp.get_instances())
        self.inObjList.extend(temp.get_lines())
        return
            ...
            if i.get_label() == label:
                for j in i.get_words():
                    if j in wordDict.keys():
                        ...
                        wordDict[j] = 1
        maxFreq = max(wordDict.values())
        wordList = []
        while num > len(wordList):
            maxFreq = max(wordDict.values())
            for i in wordDict.keys():
                if maxFreq == wordDict[i]:
                    wordList.append(i)
                    wordDict[i] = 0
        self.set_target_words(wordList)
        return
